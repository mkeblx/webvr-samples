...
if ('getVRDisplays' in navigator && !('VRFrame' in window)) {
  ...
  VRDisplay.prototype.updateFrame = (function() {
    ...
    function mat4_perspectiveFromFieldOfView(out, fieldOfView, ) {
    ...
    function mat4_viewFromPoseOffset(out, pose, offset) {
      var orientation = pose.orientation;
      var position = pose.position;
      if (!orientation) { orientation = defaultOrientation; }
      if (!position) { position = defaultPosition; }

      mat4_fromRotationTranslation(out, orientation, position);
      mat4_translate(out, out, offset);
      mat4_invert(out, out);
    }

    function updateEyeTransform(eye, pose, parameters, near, far) {
      mat4_perspectiveFromFieldOfView(eye.projectionMatrix, parameters.fieldOfView, near, far);
      mat4_viewFromPoseOffset(eye.viewMatrix, pose, parameters.offset);
    }
    ...
    return function(vrFrame) {
      ...
      updateEyeTransform(vrFrame.leftEye, pose, this.getEyeParameters("left"), this.depthNear, this.depthFar);
      updateEyeTransform(vrFrame.rightEye, pose, this.getEyeParameters("right"), this.depthNear, this.depthFar);
